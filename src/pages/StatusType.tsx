...
// Local interface for DefectStatus
...
const StatusType: React.FC = () => {
  ...
  const paginatedStatusTypes = statusTypes.slice(
  ...
  useEffect(() => {
    if (isCreateModalOpen || isEditModalOpen) {
      const isDuplicate = statusTypes.some(
        (s) =>
          s.colorCode.toLowerCase() === formData.colorCode.toLowerCase() &&
  ...
  const validateForm = () => {
    ...
    if (!/^[A-Za-z ]+$/.test(formData.defectStatusName.trim())) {
      return {
        isValid: false,
        message: "Status Name can only contain alphabets and spaces.",
      };
    }
  ...
  const handleCreate = () => {
    ...
    const exists = statusTypes.some(
      (s) =>
        s.defectStatusName.trim().toLowerCase() ===
        formData.defectStatusName.trim().toLowerCase(),
    );
    if (exists) {
    ...
    const newStatus: DefectStatus = {
      id:
        statusTypes.length > 0
          ? Math.max(...statusTypes.map((s) => s.id)) + 1
          : 1,
      defectStatusName: formData.defectStatusName,
  ...
  const handleEdit = () => {
    ...
    if (
      formData.defectStatusName.trim() ===
        editingStatus.defectStatusName.trim() &&
    ...
    const normalizedColor = normalizeColor(formData.colorCode);
    const updatedStatusTypes = statusTypes.map((s) =>
      s.id === editingStatus.id
        ? {
            ...s,
            defectStatusName: formData.defectStatusName,
      ...
      >
        <div>
          <p>
            ...
            <strong>{deletingStatus?.defectStatusName}</strong>"?
          </p>
